...
function validateParams (flags, indexUrlFlag, indexDirFlag, indexSourceFlag) {
  console.log('---->', flags, indexUrlFlag, indexDirFlag, indexSourceFlag);

  const indexLessonsFlag = getFlagIndex(flags.lessons);

  // console.log(indexUrlFlag == -1
  //   , indexUrlFlag === process.argv.length - 1
  ...
  //indexUrlFlag == -1 ||
  if (indexUrlFlag === process.argv.length - 1
    || indexDirFlag === process.argv.length - 1
    || indexSourceFlag === process.argv.length - 1
    || indexLessonsFlag === process.argv.length - 1
    || isNextFlag(flags, process.argv[indexUrlFlag + 1])
    || isNextFlag(flags, process.argv[indexDirFlag + 1])
    || isNextFlag(flags, process.argv[indexSourceFlag + 1])
    || isNextFlag(flags, process.argv[indexLessonsFlag + 1]))
    printUsage();
...
function printUsage () {
  ...
  console.log(`usage: node ${filename} -s source-url -u course-url [-d dirname] [-l lessons]`.yellow);
  console.log(`  -u, --url: https://coursehunter.net/course_name`.yellow);
  console.log(`  -s, --url: https://coursehunter.net/source/source_name`.yellow);
