...
function objectSelectorScrape(rootNode, locationUrl, selectors) {
  return Object.keys(selectors).reduce((result, selectorKey) => {
    result[selectorKey] = scrape(rootNode, locationUrl, selectors[selectorKey]);
    return result;
  }, {});
}
...
function multiSelectorScrape(dom, locationUrl, rootSelector, selectors) {
  const rootNode = scrape(dom, locationUrl, rootSelector);
  return Object.keys(selectors).reduce((result, selectorKey) => {
    result[selectorKey] = scrape(rootNode, locationUrl, selectors[selectorKey]);
    return result;
  }, {});
...
function has(element, selector) {
  return !!createNode([element]).scrape(selector);
}
...
function createNode(elements, locationUrl) {
  ...
  elements.has = selector => has(elements[0], selector);
