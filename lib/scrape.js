...
const getCourses = async ({ token, allPages, zip, code }) => {
  ...
  let allCourses = await fetcher
    ...
    .filter(c => c);
  scrapingMsg.succeed(`1111Courses gathered: ${counter}`)
  /*return {
...
const scrapeSelectively = ({ email, password, url, downDir, subtitle, zip, code }) => Promise
  .resolve()
  .then(async () => {
...
const scrapeAll = ({ email, password, type, url, downDir, subtitle, zip, code }) => Promise
  .resolve()
  .then(async () => {
    return getToken(email, password);
  })
