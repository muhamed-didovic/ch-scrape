...
const prompt = async (options = {}) => {
  ...
  if (input.length === 0 && searchOrDownload === false) {
    input.push(await askOrExit({
      ...
      initial : 'https://coursehunter.net/course/posledniy-kurs-po-algoritmam-kotoryy-vam-ponadobitsya', //https://coursehunter.net/source/developedbyed-com',
      validate: value => value.includes('coursehunter.net') ? true : 'Url is not valid'
    }))
  } else {

    let searchCoursesFile = false;
    if (fs.existsSync(path.resolve(__dirname, '../json/search-courses.json'))) {
