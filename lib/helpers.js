  ...
  const allCourseWithVideosLessons = await Promise
    ...
    .map(allCourses, async (course) => {
      ...
      if (subtitle === 'yes' || videos === 'yes') {
        ...
        lessonsMsg.text = `Collecting course for: https://coursehunter.net/api/v1/course/${course.id}/lessons`
        ...
        (async () => {
          ...
          if (subtitle === 'yes') {
            ...
            course.subtitles = lessonsData
              .filter(lesson => lesson.subtitle.includes('http') && lesson.subtitle.includes('.net/'))
        ...
        (async () => {
          ...
          if (videos === 'yes') {
            ...
            course.names = lessonsData.map(lesson => {
              let str = lesson.title.replace(
                /\s\|\s\d{2}:\d{2}:\d{2}/g,
                ""
              );

              const parts = str.split(' ');  // Split the title into parts
              const number = parts.shift();  // Remove the number from the beginning

              parts.splice(0, 0, `${number}.`);  // Insert the number with a period
