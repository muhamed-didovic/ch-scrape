...
const putCoursesIntoFile = async (allCourseWithVideosLessons, downDir) => {
  ...
  fs.writeFileSync(path.join(downDir, downloads), JSON.stringify(allCourseWithVideosLessons, null, 2), 'utf8');
  fileMsg.succeed(`Videos collected in: ${path.join(downDir, downloads)}`)
  return {
