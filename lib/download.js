...
const putCoursesIntoFile = async (allCourseWithVideosLessons, downDir) => {
  ...
  fs.writeFileSync(path.join(downDir, path.sep, downloads), JSON.stringify(allCourseWithVideosLessons, null, 2), 'utf8');
  fileMsg.succeed(`Videos collected in: ${path.join(downDir, path.sep, downloads)}`)
  return {
    courses : allCourseWithVideosLessons,
    fileName: path.resolve(downDir, path.sep, downloads)
  };
};
